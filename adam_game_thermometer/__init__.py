...
class C(BaseConstants):
    NAME_IN_URL = 'adam_game_thermometer'
...
class Page1(Page):
    ...
    def live_method(player: Player, data):
        ...
        strings = [str(i) for i in range(1, 101)]
        for string in strings:
            if button == string:
                player.open_order += button
                if len(player.open_time) == 0:
                    player.open_time += current_time
                else:
                    player.open_time += ',' + current_time
            if  like == string:
                # One like per box opened
                if len(player.open_order) - 1 >= len(player.like_order):
                    if len(player.like_order) == 0:
                        player.like += like
                    if len(player.like_order) > 0:
                        player.like += ',' + like
